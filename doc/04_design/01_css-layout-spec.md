...
- フォーム系コントロール（ボタン・テキストボックス・プルダウン）は `theme.css` の `--control-height`（40px）で縦幅を統一する
...
- `#schedule-table-wrapper`: `max-height` を持つスクロールコンテナ（`overflow-x: auto`, `overflow-y: auto`）
- `#schedule-table`: `width: max-content`
- ヘッダ固定（縦スクロール時も表示維持）
- 土日の日付表示: 土曜は青文字、日曜は赤文字
- 土日のセル背景: グレー
- グループ編集・ユーザメンテナンス一覧も `.table-card` を縦スクロール化し、`th` を `position: sticky` で固定表示
- ユーザメンテナンス画面では一覧上部に検索カードを配置し、ユーザID/ユーザ名/メール/ロールの条件入力で絞り込み
...
- 所属ユーザは2列グリッドで選択性を向上。
- 所属ユーザ欄には絞り込み入力と「絞り込み結果を全選択」ボタンを配置する。
